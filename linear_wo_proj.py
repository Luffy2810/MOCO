...
def Linear():
    ...
    if not os.path.exists('linear_wo_proj'):
        os.makedirs('linear_wo_proj')
    ...
    for epoch in range(num_epochs_linear):
        ...
        if LINEAR_TRAINING:
            ...
            plt.savefig('linear_wo_proj/losses.png')
            plt.close()
            ...
            plt.savefig('linear_wo_proj/accuracy.png')
            plt.close()
            ...
            if test_acc >= max_test_acc:
                ...
                torch.save(linear_classifier.state_dict(), 'linear_wo_proj/model.pth')
                torch.save(linear_optimizer.state_dict(), 'linear_wo_proj/optimizer.pth')


        np.savez("linear_wo_proj/linear_losses_train_file", np.array(losses_train_linear))
        np.savez("linear_wo_proj/linear_losses_test_file", np.array(losses_test_linear))
        np.savez("linear_wo_proj/linear_acc_train_file", np.array(acc_train_linear))
        np.savez("linear_wo_proj/linear_acc_test_file", np.array(acc_test_linear))
    torch.save(linear_classifier,'linear_wo_proj.pt')
    if(os.path.isfile("linear_wo_proj/model.pth")):
        linear_classifier.load_state_dict(torch.load("linear_wo_proj/model.pth"))
    linear_classifier.eval()
